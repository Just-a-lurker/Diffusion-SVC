...
print("Loading full dataset (train + test + valid)...")

ds_train = load_dataset(DATASET_NAME, split="train")
ds_test  = load_dataset(DATASET_NAME, split="test")
ds_valid = load_dataset(DATASET_NAME, split="valid")

# gộp tất cả lại
ds = concatenate_datasets([ds_train, ds_test, ds_valid])

print(f"Total samples before filter: {len(ds)}")
