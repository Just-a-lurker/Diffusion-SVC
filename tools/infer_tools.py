...
class DiffusionSVC:
    ...
    @torch.no_grad()
    def extract_f0(self, audio, key=0, sr=44100, silence_front=0, HZ_MAX = 0, HZ_MIN = 0 ,LOG_HZ_MIN= 0, LOG_HZ_MAX=0, manual_semitone=None):
        ...
        f0 = shift_f0_contour(f0, HZ_MAX=HZ_MAX, HZ_MIN=HZ_MIN, max_semitone=20, manual_semitone=manual_semitone)
        # f0 = compress_low_f0(f0, LOG_HZ_MAX = LOG_HZ_MAX, LOG_HZ_MIN = LOG_HZ_MIN, gammaLow=0.7, gammaHigh=0.7)
    ...
    def infer_from_long_audio(self, audio, sr=44100, key=0, spk_id=1, spk_mix_dict=None, aug_shift=0,
                              infer_speedup=10, method='dpm-solver', k_step=None, use_tqdm=True,
                              spk_emb=None,
                              threhold=-60, threhold_for_split=-40, min_len=5000, index_ratio=0, HZ_MAX=0, HZ_MIN = 0, LOG_HZ_MIN=0, LOG_HZ_MAX=0, manual_semitone = None):
        ...
        f0 = self.extract_f0(audio, key=key, sr=sr, HZ_MAX=HZ_MAX,HZ_MIN=HZ_MIN ,LOG_HZ_MIN=LOG_HZ_MIN, LOG_HZ_MAX=LOG_HZ_MAX, manual_semitone=manual_semitone)
        volume, mask = self.extract_volume_and_mask(audio, sr, threhold=float(threhold))
    ...
    def infer_from_audio_for_realtime(self, audio, sr, key, spk_id=1, spk_mix_dict=None, aug_shift=0,
                                      ...
                                      index_ratio=0, use_hubert_mask=False, HZ_MAX=0, HZ_MIN = 0, LOG_HZ_MIN=0, LOG_HZ_MAX=0, manual_semitone=0):
        ...
        f0 = self.extract_f0(audio, key=key, sr=sr, silence_front=silence_front, HZ_MAX=HZ_MAX,HZ_MIN=HZ_MIN ,LOG_HZ_MIN=LOG_HZ_MIN, LOG_HZ_MAX=LOG_HZ_MAX, manual_semitone=manual_semitone)
        _f0_end_time = time.time()
