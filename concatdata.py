from datasets import concatenate_datasets
from datasets import load_dataset #36
import soundfile as sf
import os
import random
DATASET_NAME = "nguyendv02/ViMD_Dataset"
TRAIN_DIR = "data/train/audio/2"
VAL_DIR   = "data/val/audio/2"
VAL_SIZE  = 10
SEED = 42

random.seed(SEED)
os.makedirs(TRAIN_DIR, exist_ok=True)
os.makedirs(VAL_DIR, exist_ok=True)
dataset = load_dataset(DATASET_NAME)  # không chỉ định split
print(dataset)
all_ds = concatenate_datasets([dataset[s] for s in dataset.keys()])
print(f"Total samples (all splits combined): {len(all_ds)}")
def filter_fn(x):
    return x["region"] == "Central" and x["gender"] == 1

ds = all_ds.filter(filter_fn)
print(f"Samples after filter: {len(ds)}")

num_samples = len(ds)
val_indices = set(random.sample(range(num_samples), VAL_SIZE))

train_idx = 0
val_idx = 0

for i, sample in enumerate(ds):
    audio = sample["audio"]
    wav = audio["array"]
    sr = audio["sampling_rate"]

    if i in val_indices:
        out_path = os.path.join(VAL_DIR, f"{val_idx:04d}.wav")
        val_idx += 1
    else:
        out_path = os.path.join(TRAIN_DIR, f"{train_idx:04d}.wav")
        train_idx += 1

    sf.write(out_path, wav, sr)

print(f"Done. Train: {train_idx}, Val: {val_idx}")
